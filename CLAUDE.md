...
- **マイクで自動判定→自動でnextChord()機能は2026-07-31に実装を削除しリセット済み**: 実機での判定精度が不安定だったため、旧実装（フッター🎤ボタン・オンセット検出・FFTクロマベクトル判定一式）を`index.html`から完全に削除した。現時点でこのアプリにマイク関連のコード・UIは存在しない。再設計にあたっての思想の掘り下げ・文献調査・リスク洗い出し・マイルストーンは`作業日記\01 ギター練習ツール\プラン\2026-08-06_マイク自動判定機能_再設計ロードマップ.md`にまとめてある（旧実装の経緯・教訓は`2026-07-31_...実装リセットと教訓.md`、さらに旧旧計画書`2026-07-21_...実装計画.md`も参照用に残存）。この機能に再着手する時は、まずロードマップを読んでから設計すること。再実装時はDSPロジック（ピーク検出・放物線補間・倍音畳み込み・コード一致スコアリング）を`mic-chord-match.js`という別ファイルに切り出す方針（アプリ本体`index.html`はUI・状態管理のみに保つ）で、`依存ライブラリなし`の方針を崩さない`<script src>`読み込みで対応する予定（既存の単一ファイル構成に対する唯一の例外）。
