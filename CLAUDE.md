...
- **「流れる」タブ**（コードタブとは別画面）: 個別の小さいフレット図を並べるのではなく、**6弦ぶんの弦線を1本の帯として`FLOW_N`(4)コードぶん通しで描き（`drawFlowStaff`、`<svg id="flow-staff">`1枚）、その上に各コードの運指を重ねて描く**（現在のコードは`<rect>`の枠で強調）。左端の弦名(`#flow-string-labels`)は固定表示で、水平方向のスクロールでは動かない（縦位置だけがSVGの実寸と一致するようCSS側で`27.86vw`等、`flow-staff`と同じ比率の計算式を使っている。ここがズレると弦名と実際の弦線の高さが合わなくなるので、`FLOW_H`/`FLOW_STR_TOP`/`FLOW_STR_BOTTOM`を変えたらCSS側の比率も直すこと）。位置は毎フレーム`layoutFlow()`が`.flow-track`に`transform: translate()`で計算し直す方式（SVGの内容は`syncFlowQueue()`→`drawFlowStaff()`でチード変更の瞬間だけ再構築、各コードの`<g class="flow-slot">`の不透明度だけ毎フレーム更新）。位置計算式（`-(0.5+frac)*w`、`FLOW_REST_OPACITY`での不透明度補間も同様）は切替の瞬間に見た目がジャンプしないよう数学的に連続になるよう設計してある。小節ベース自動切替(`barsPerSwitch>0`)かつメトロノーム再生中のみ流れ、それ以外（手動送りのみ）は静止表示になる。マーカー線(`#flow-marker`)は`sched()`が実際に音を鳴らす拍タイミングで`pulseFlowMarker()`により毎拍光り、その拍でコード切替が起きるかを`sched()`側で先読み(`willSwitch`)して`onBeatUI`に渡すことで、切替の拍だけ大きく光らせて区別している。
