...
- **マイクで自動判定→自動でnextChord()機能は2026-07-31に実装を削除しリセット済み**: 実機での判定精度が不安定だったため、旧実装（フッター🎤ボタン・オンセット検出・FFTクロマベクトル判定一式）を`index.html`から完全に削除した。設計は再検討中で、現時点でこのアプリにマイク関連のコード・UIは存在しない。経緯・技術的な教訓・旧実装の詳細は`作業日記\01 ギター練習ツール\プラン\2026-07-31_マイク自動判定機能_実装リセットと教訓.md`（旧計画書`2026-07-21_マイク自動判定機能実装計画.md`も参照用に残存）を参照。この機能に再着手する時は、まずそのメモを読んでから設計すること。
